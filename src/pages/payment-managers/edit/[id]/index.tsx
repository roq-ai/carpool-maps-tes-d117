...
import { getPaymentManagerById, updatePaymentManagerById } from 'apiSdk/payment-managers';
...
function PaymentManagerEditPage() {
  ...
  const handleSubmit = async (values: PaymentManagerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePaymentManagerById(id, values);
      mutate(updated);
      resetForm();
